...
class UserManager(models.Manager):
    def registerVal(self, postData):
        ...
        if not postData['dob']:
            results['status']=False
            results['errors'].append("Please enter your birthdate.")
        if postData['dob']:
            today = datetime.date.today()
            todayString = datetime.datetime.strftime(today, '%Y-%m-%d')
            yearString = todayString[:4]
            year = int(yearString)
            birthYear = postData['dob'][:4]
            if int(birthYear)+18 > year:
                results['status']=False
                results['errors'].append("You must be 18 years old to use this site.")
        if results['status']==True:
            ...
            if len(user) != 0:
                ...
                User.objects.create(name=postData['name'], alias=postData['alias'], email=postData['email'].lower(), password=password, dob=postData['dob'])
                results['errors'].append("Registration successful, please log in.")
                return results
        return results

    def loginVal(self, postData):
        results = {'status':True, 'errors':[], 'user':{}}
